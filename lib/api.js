...
async function saveToStorage(stream) {
    ...
    const response = await fetch(`${REGISTRY_API_URL}/api/storage`, {
        method: 'POST',
        body: form,
...
async function addModuleToRegistry(uri, account, key) {
    const response = await fetch(`${REGISTRY_API_URL}/api/registry/${account}/add-module?uri=${uri}&key=${key}`, {
        method: 'POST'
    });
...
async function addSiteBinding(name, branch, site, account, key) {
    const response = await fetch(`${REGISTRY_API_URL}/api/registry/${account}/add-site-binding?name=${name}&branch=${branch}&site=${site}&key=${key}`, {
        method: 'POST'
    });
