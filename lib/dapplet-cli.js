...
async function deploy(registry, account, key, options) {
    ...
    for (let i = 0; i < dirs.length; i++) {
        ...
        await _deployFromDirectory(registry, account, key, dir);
    }
}
...
async function _deployFromDirectory(registry, account, key, dir) {
    ...
    const distUri = await saveToStorage(registry, distStream, account);
    ...
    const manifestUri = await saveToStorage(registry, tempManifestStream, account);
