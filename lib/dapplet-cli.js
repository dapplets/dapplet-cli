...
async function deploy(account, key, options) {
    const dirs = ['build'];

    if (options.archive === true) {
        const versions = fs.readdirSync(ARCHIVE_DIRECTORY_NAME);
        console.log(`Found ${versions.length} archive modules.`);
    ...
    for (let i = 0; i < dirs.length; i++) {
        const dir = dirs[i];
        console.log(`[${i + 1}/${dirs.length}] Deploying of ${dir} directory.`);
        await _deployFromDirectory(account, key, dir);
    }
}
...
async function _deployFromDirectory(account, key, dir) {
    ...
    const distUri = await saveToStorage(distStream, account);
    ...
    const manifestUri = await saveToStorage(tempManifestStream, account);
