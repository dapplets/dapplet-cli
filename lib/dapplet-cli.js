...
const { start } = require('./copyTemplate');
...
async function _deployFromDirectory(registry, account, key, dir, options) {
    ...
    console.log(`Dist URI: ${distUri}. Hash: ${distHash}`);
    ...
    const tempManifestBuffer = fs.readFileSync(tempManifestPath);
    const manifestHash = ethers.utils.keccak256(new Uint8Array(tempManifestBuffer)).substring(2);
    ...
    console.log(`Manifest URI: ${manifestUri}. Hash: ${manifestHash}`);
    fs.unlinkSync(tempManifestPath);

    const hashUris = [{ hash: manifestHash, uri: manifestUri }, { hash: distHash, uri: distUri }];
    await addModuleWithObjects(registry, account, name, branch, version, hashUris, key);
